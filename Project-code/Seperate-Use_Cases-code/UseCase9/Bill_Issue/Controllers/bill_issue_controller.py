...
class BillController:
    ...
    def check_if_bill_issued(self, selected_treats, selected_prescs):
        ...
        csv_path = os.path.join(parent_dir, "archive (1)", "final_billing4.csv")
    ...
    def save_bill(self, selected_treats, selected_prescs, screen_amount_passed):
        ...
        try:
            parent_dir = Path.cwd().parent.parent
            path = os.path.join(parent_dir, "archive (1)", "final_treatments4.csv")

            # 2. Load existing records to maintain sequential bill_id integrity
            if os.path.exists(path) and os.path.getsize(path) > 0:
                df_existing = pd.read_csv(path)
                # Safely extract the highest numerical suffix from bill_id to avoid key duplication
                try:
                    max_id = df_existing['bill_id'].str.extract(r'B(\d+)').astype(int).max()[0]
                    next_id_num = int(max_id) + 1
                except:
            ...
            if self.insurance_discount_ratio > 0:
                ...
                payment_method = "Cash"  # Default standard fallback

            # 4. Insert an itemized row for each selected treatment
            for t_id in selected_treats:
                item = next((t for t in self.treatments if t['id'] == t_id), None)
                if item:
                    # Final cost for this item = item_cost * (1 - discount_ratio)
                    final_item_cost = item['cost'] * (1 - self.insurance_discount_ratio)
                    ...
                    new_rows.append({
                        ...
                        'payment_method': payment_method,
                        'payment_status': 'Pending',
                        'prescription_id': None  # Treatment row, prescription remains blank
                    })
                    next_id_num += 1
            ...
            for p_id in selected_prescs:
                ...
                if item:
                    ...
                    new_rows.append({
                        'bill_id': f"B{next_id_num:03d}",
                        'patient_id': self.patient_id,
                        'treatment_id': None,  # Prescription row, treatment remains blank
                        'bill_date': bill_date_str,
                        'amount': round(final_item_cost, 2),
                        'payment_method': payment_method,
                        'payment_status': 'Pending',
                        'prescription_id': p_id  # Prescription field populated at the end
                    })
                    next_id_num += 1
            ...
            messagebox.showinfo("Success", msg)
            self.on_back_click()  # Redirect user safely back to previous workspace
